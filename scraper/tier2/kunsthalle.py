"""
kunsthalle.py – DiesDasDüsseldorf
Scraper für die Kunsthalle Düsseldorf: Ausstellungen und Veranstaltungen.

Strategie:
1. Primär: JSON-LD structured data (schema.org/Event, ExhibitionEvent, VisualArtsEvent)
2. Fallback: HTML-Parsing der Veranstaltungs-Liste

URL: https://www.kunsthalle-duesseldorf.de/ausstellungen-und-programm/
...
BASE_URL = "https://www.kunsthalle-duesseldorf.de"
LISTE_URL = "https://www.kunsthalle-duesseldorf.de/ausstellungen-und-programm/"
QUELLE_NAME = "Kunsthalle Düsseldorf"
ORT_STANDARD = "Kunsthalle Düsseldorf"
...
KATEGORIE_MAPPING = {
    ...
    "kino": "kultur", "film": "kultur", "performance": "kultur",
    ...
    for script in soup.find_all("script", type="application/ld+json"):
        ...
        for eintrag in eintraege:
            if not isinstance(eintrag, dict):
                continue
            if eintrag.get("@type") not in ("Event", "ExhibitionEvent", "VisualArtsEvent", "MusicEvent"):
                continue
...
def _events_aus_html(soup: BeautifulSoup, heute: date) -> list[dict]:
    """
    Fallback: HTML-Parsing der Veranstaltungs-Karten.

    Sucht nach typischen Ausstellungs-/Event-Cards mit Klassen wie
    .event, .exhibition, .program-item, .veranstaltung oder article-Tags.
    ...
    # Kandidaten-Selektoren für Event-Karten
    kandidaten = (
        soup.select(".event")
        or soup.select(".exhibition")
    ...
    if not kandidaten:
        logger.warning("HTML-Fallback: Keine Event-Karten gefunden")
        return []
    ...
    for karte in kandidaten:
        ...
        # Falls kein explizites Datum-Tag: gesamten Karten-Text durchsuchen
        if not datum_text:
        ...
        if beschreibung_tag:
            beschreibung = beschreibung_tag.get_text(strip=True)[:300]
        else:
            beschreibung = None
...
def scrape() -> list[dict]:
    """
    Scrapt Ausstellungen und Veranstaltungen der Kunsthalle Düsseldorf.

    Strategie:
    1. JSON-LD structured data
    2. HTML-Parsing als Fallback
