...
# GraphQL Query
EVENTS_QUERY = """
query GET_SALON_EVENTS($filters: FilterInputDtoInput, $pageSize: Int, $page: Int) {
    ...
    try:
        with httpx.Client(headers=headers, follow_redirects=True, timeout=30) as client:
            while seite <= 5:
                ...
                # Nur Salon-Events filtern (Venue-ID 15478)
                salon_listings = [
                    l for l in listings
                    if str((l.get("event") or {}).get("venue", {}).get("id", "")) == str(RA_VENUE_ID)
                ]
                logger.debug(
                    "Seite %d: %d Gesamt-Listings, %d Salon-Events",
                    seite, len(listings), len(salon_listings)
                )

                for listing in salon_listings:
                    try:
                        ...
                        logger.error("Fehler bei Salon-Event-Verarbeitung: %s", err)
                        continue
