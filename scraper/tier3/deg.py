"""
deg.py – DiesDasDüsseldorf
Playwright-Scraper für DEG Eishockey Heimspiele Düsseldorf.

Scrapt nur Heimspiele aus dem offiziellen Spielplan der Düsseldorfer EG.

Strategie:
1. Primär: JSON-LD structured data (schema.org/SportsEvent)
2. Fallback: HTML-Parsing der Spielplan-Tabelle

URL: https://www.deg-hockey.de/spielplan
...
LISTE_URL = "https://www.deg-hockey.de/spielplan"
BASE_URL = "https://www.deg-hockey.de"
QUELLE_NAME = "DEG Eishockey Düsseldorf"
ORT_STANDARD = "PSD BANK DOME Düsseldorf"
ADRESSE_STANDARD = "Siegburger Str. 15, 40591 Düsseldorf"
KATEGORIE = "sport"
PREIS_FALLBACK = "ab 10€"

# Auswärtsspiel-Identifier – wenn ein fremder Ort genannt wird
AUSWAERTS_BEGRIFFE = ["auswärts", "away", "gastspiel"]
# Heimspiel-Indikatoren
HEIM_BEGRIFFE = ["psd bank dome", "dome", "düsseldorf", "duesseldorf", "deg", "heimspiel", "home"]
...
def _ist_heimspiel(ort_text: str, karten_text: str = "") -> bool:
    """
    Prüft ob ein Spiel ein Heimspiel ist.

    Args:
        ort_text: Ortsbezeichnung des Events
        karten_text: Volltext der Event-Karte

    Returns:
        True wenn Heimspiel, sonst False
    """
    kombiniert = (ort_text + " " + karten_text).lower()
    if any(begriff in kombiniert for begriff in AUSWAERTS_BEGRIFFE):
        return False
    if any(begriff in kombiniert for begriff in HEIM_BEGRIFFE):
        return True
    return True  # Im Zweifel Heimspiel annehmen
    ...
    for script in soup.find_all("script", type="application/ld+json"):
        ...
        for eintrag in eintraege:
            if not isinstance(eintrag, dict):
                continue
            if eintrag.get("@type") not in ("Event", "SportsEvent", "IceHockeyGame"):
                continue
...
def _events_aus_html(html: str, heute: date, enddatum: date) -> list[dict]:
    ...
    selektoren = [
        ...
        "li[class*='game']", "tr[class*='game']",
        ".schedule-item", ".spielplan-eintrag",
    ]
    ...
    for karte in karten:
        try:
            karte_text = karte.get_text(separator=" ", strip=True)

            # Auswärtsspiel überspringen
            if not _ist_heimspiel("", karte_text):
                continue
...
async def scrape() -> list[dict]:
    """
    Scrapt DEG Eishockey Heimspiele vom offiziellen Spielplan.
