"""
sport_im_park.py – DiesDasDüsseldorf
Playwright-Scraper für das kostenlose Sportangebot "Sport im Park" der Stadt Düsseldorf.

Strategie:
1. Primär: JSON-LD structured data (schema.org/Event)
2. Fallback: HTML-Parsing der Veranstaltungsliste

URL: https://www.duesseldorf.de/sportamt/sport-im-park
...
LISTE_URL = "https://www.duesseldorf.de/sportamt/sport-im-park"
BASE_URL = "https://www.duesseldorf.de"
QUELLE_NAME = "Sport im Park Düsseldorf"
KATEGORIE_STANDARD = "sport"
PREIS_STANDARD = "kostenlos"
...
def _kategorie_erkennen(text: str) -> str:
    """
    Erkennt die passende Kategorie anhand des Titels.
    ...
    outdoor_begriffe = ["wandern", "radeln", "fahrrad", "natur", "park", "outdoor", "spazier"]
    for begriff in outdoor_begriffe:
    ...
    for script in soup.find_all("script", type="application/ld+json"):
        ...
        for eintrag in eintraege:
            if not isinstance(eintrag, dict):
                continue
            if eintrag.get("@type") not in ("Event", "SportsEvent"):
                continue
            ...
            if isinstance(ort_roh, dict):
                ort = ort_roh.get("name", QUELLE_NAME)
...
def _events_aus_html(html: str, heute: date, enddatum: date) -> list[dict]:
    ...
    selektoren = [
        ...
        ".event-item", "article[class*='event']",
        "div[class*='event']", ".veranstaltung", ".sport-event",
        "article", ".teaser",
    ]
    ...
    for karte in karten:
        try:
            ...
            logger.error("Fehler beim Parsen einer Karte: %s", str(fehler))
            continue
...
async def scrape() -> list[dict]:
    """
    Scrapt kostenlose Sport-im-Park-Events der Stadt Düsseldorf.
